...
void ReadAnalyticSolution(const Configuration & conf, cube_t & data, const std::string & filename)
{
    ...
    for (int t = 0; file >> t >> physical_time;) {      // header line contains a time stamps
        ...
        for (int y = 0; y < GLOBAL_NELEMS_Y; ++y) {
            ...
            file >> i >> j >> val;
            if (!((i == x) && (j == y))) {
                assert_true(0) << "mismatch between the grid layouts" << endl;
            }
            data[{i,j,t}] = val;
        }}
    }
...
//-------------------------------------------------------------------------------------------------
void GetObservations(field_grid_t & field, const cube_t & analytic_sol, int timestep)
{
                ...
                bool print_image = true, gnuplot::Gnuplot * gp = nullptr)
{
    ...
    pfor(Origin, SubDomGridSize, [&](const point2d_t & idx) {
        ...
        for (int y = 0; y < NELEMS_Y; ++y) { int yg = Sub2GloY(idx, y);
            int pos = xg * GLOBAL_NELEMS_Y + yg;
            CheckRange1D(pos, ImageSize);
...
double SchwarzUpdate(const Configuration & conf,
                      boundary_grid_t & boundaries, domain_t & domain, double physical_time)
{
    ...
    // Compute flow given a time.
    std::pair<double,double> flow = Flow(conf, physical_time);
    ...
    {
        Boundary & bnd = boundaries[idx];
        bnd.outer[dir] = is_outer;
        bnd.inflow[dir] = false;
        ...
        if (normal_x * flow.first + normal_y * flow.second < 0) {
            bnd.inflow[dir] = true;
            bnd.myself = domain[idx].getBoundary(dir);
            bnd.remote = domain[idx + remote_idx[dir]].getBoundary(remote_dir[dir]);
            domain[idx].setBoundary(dir, bnd.remote);

            // Compute and accumulate aggregated difference between subdomains' borders.
            double rsum = 0.0, msum = 0.0, diff = 0.0;
            assert_true(bnd.myself.size() == bnd.remote.size());
            for (auto r = bnd.remote.begin(),
                      m = bnd.myself.begin(); m != bnd.myself.end(); ++m, ++r) {
                diff += std::fabs(*r - *m);
                rsum += std::fabs(*r);
                msum += std::fabs(*m);
            }
    ...
    // Update subdomain boundaries in the whole domain in parallel.
    ...
    pfor(Origin, SubDomGridSize, [&](const point2d_t & idx) {
        ...
        boundaries[idx].rel_diff = numer_sum / std::max(denom_sum, TINY);
        glo_diff[idx] = boundaries[idx].rel_diff;
    });
    double rel_diff = ReduceAbsMax(glo_diff);
    std::cout << "Max. relative difference at subdomain borders: " << rel_diff << endl << flush;
    return rel_diff;
}
double SchwarzUpdate(const Configuration & conf,
