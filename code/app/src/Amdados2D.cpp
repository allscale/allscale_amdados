...
#include "amdados/app/utils/reduction.h"
...
//-----------------------------------------------------------------------------
// Function gets the observations at certain time and returns them for each
// subdomain. Note, ordinate changes faster than abscissa (row-major stacking
// in 2D array "field(x,y)").
//-----------------------------------------------------------------------------
void GetObservations(Matrix & subfield, const point2d_t & idx,
                     const cube_t & analytic_sol, int timestep)
{
    assert_true(subfield.NRows() == NELEMS_X);
    assert_true(subfield.NCols() == NELEMS_Y);
    for (int x = 0; x < NELEMS_X; ++x) { int xg = Sub2GloX(idx, x); // global x
    for (int y = 0; y < NELEMS_Y; ++y) { int yg = Sub2GloY(idx, y); // global y
        // TODO: range checking
...
//-----------------------------------------------------------------------------
// Function (optionally) writes the whole state field into a file in binary,
// grayscaled "pgm" format, where all the values are adjusted to [0..255]
// interval. Function also optionally plots the state field using Gnuplot.
//-----------------------------------------------------------------------------
void ShowImage(ImageWriter & writer, const Grid<Matrix,2> & field,
               ...
               bool write_image = true, gnuplot::Gnuplot * gp = nullptr)
{
    ...
    const std::vector<unsigned char> & img =
            writer.Write(title, field, time_index, write_image);
    if (gp != nullptr) { // plot the image with Gnuplot, if available
        char title[64];
        snprintf(title, sizeof(title)/sizeof(title[0]),
                    "frame%05d", time_index);
        gp->PlotGrayImage(img.data(), GLOBAL_NELEMS_X,
                                      GLOBAL_NELEMS_Y, title, true);
    }
}
...
// in the whole domain.
//-----------------------------------------------------------------------------
void WriteImageOfSensors(ImageWriter & writer, const Grid<Matrix,2> & H)
{
    Grid<Matrix,2> field(SubDomGridSize);
    pfor(Origin, SubDomGridSize, [&](const point2d_t & idx) {
        field[idx].Resize(NELEMS_X, NELEMS_Y);

        Matrix       & subfield = field[idx];
        const Matrix & subH = H[idx];

        // Everything is set to black except border points painted in dark gray.
        Fill(subfield, 0.0);
        for (int x = 0; x < NELEMS_X; ++x) { subfield(x,0) =
                                             subfield(x,NELEMS_Y-1) = 128.0; }
        for (int y = 0; y < NELEMS_Y; ++y) { subfield(0,y) =
                                             subfield(NELEMS_X-1,y) = 128.0; }

        // Sensor locations are depicted in white.
        for (int r = 0; r < NUM_SUBDOMAIN_OBSERVATIONS; ++r) {
        for (int c = 0; c < SUB_PROBLEM_SIZE;           ++c) {
            if (subH(r,c) > 0.01) {           // H is a matrix of 0/1 values
                div_t res = div(c, NELEMS_Y); // convert plain index into (x,y)
                int x = res.quot;
                int y = res.rem;
                bool ok = (Sub2Ind(x,y) == c);
                assert_true(ok);              // check index conversion
                subfield(x,y) = 255.0;
            }
        }}
    });
...
//-----------------------------------------------------------------------------
void ComputeH(const Configuration &, Matrix & H, const point2d_t & idx)
{
...
void RunDataAssimilation(const Configuration & conf, const cube_t & analytic_sol)
{
    ...
    Grid<Matrix,2> field(SubDomGridSize);        // same state field as a grid of matrices
    Grid<Boundary,2>      boundaries(SubDomGridSize);   // boundaries of each subdomain

    Grid<KalmanFilter,2>    Kalman(SubDomGridSize);   // Kalman filters of all the subdomains
    Grid<Matrix,2> B(SubDomGridSize);        // inverse model matrices

    Grid<Matrix,2> P(SubDomGridSize);  // process model covariances of all the subdomains
    Grid<Matrix,2> H(SubDomGridSize);  // observation matrices of all the subdomains
    Grid<Matrix,2> R(SubDomGridSize);  // observation noise covariances of all the subdomains
    Grid<Matrix,2> Q(SubDomGridSize);  // process noise covariances of all the subdomains
    Grid<Vector,2> z(SubDomGridSize);  // observation vectors of all the subdomains
    ...
    pfor(Origin, SubDomGridSize, [&](const point2d_t & idx) {
        ...
        H[idx].Resize(NUM_SUBDOMAIN_OBSERVATIONS, SUB_PROBLEM_SIZE);
        R[idx].Resize(NUM_SUBDOMAIN_OBSERVATIONS, NUM_SUBDOMAIN_OBSERVATIONS);
        z[idx].Resize(NUM_SUBDOMAIN_OBSERVATIONS);
...
int Amdados2DMain()
{
    std::cout << "***** Amdados2D application *****"
    ...
    try {
        ...
        // Read analytic solution previously computed by
        // the Python application "Amdados2D.py".
        std::unique_ptr<cube_t> analytic_sol;
        analytic_sol.reset(
                new cube_t(size3d_t{GLOBAL_NELEMS_X,
                                    GLOBAL_NELEMS_Y,conf.asInt("Nt")}));
        ReadAnalyticSolution(conf, *analytic_sol,
                    conf.asString("output_dir") + "/" +
                    conf.asString("analytic_solution"));
