...
def Amdados2D(config_file, demo):
    ...
    AssurePathExists(conf.output_dir + os.sep)
    ...
    for x in range(nx):
        for y in range(ny):
            ...
            if x == 0 or x == nx-1 or y == 0 or y == ny-1:
                if ZERO_BOUNDARY_CONDITIONS:
                    # This is just a Dirichlet boundary condition; insufficient
                    # in diffusion case.
...
def InitialField(conf):
    """ Create initial ("true") field with a bell-shaped spot at some point
        and zeros elsewhere. Note, the bell-shaped spot is not very sharp to
        make the field differentiable.
    """
    nx = conf.nx
    ny = conf.ny
...
def ForwardSolver(conf, glo_idx, sensor_idx, demo):
    ...
    for k in range(Nt):
        ...
        if plt is not None:
            ...
            plt.title("True density, t=" + str(k),
